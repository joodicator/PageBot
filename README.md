...
* `!raw CMD` - [admin] send `CMD` to the server as a [https://tools.ietf.org/html/rfc2812](raw IRC message).
* `!msg TARGET MSG` - [admin] say `MSG` to the nick or channel given by `TARGET`.
* `!act TARGET MSG` - [admin] send `MSG` as a [http://www.mirc.org/mishbox/reference/ctcpref.htm](CTCP `ACTION` message) (as generated by `/me` in most IRC clients) to `target`.
