...
def relative_time_string(rtime):
    ...
    ps = 'year', 'month', 'day', 'hour', 'minute', 'second'
    for p1, p2 in izip_longest(ps, ps[1:]):
        n1 = getattr(rtime, p1 + 's')
        if n1 == 0:
            continue
        str = '%d %s%s' % (n1, p1, s(n1))
        if p2 is not None:
            n2 = getattr(rtime, p2 + 's')
            if n2 != 0:
                str += ' and %d %s%s' % (n2, p2, s(n2))
        return '\2%s\2 ago' % str
    else:
